...
// NimBLE invokes the store callbacks with ble_hs_mutex held. Taking bt_lock
// here deadlocks against tasks that hold bt_lock while calling host APIs
// (which acquire ble_hs_mutex), so the lists get their own dedicated lock and
// handlers that need bt_lock are deferred to KernelMain.
static PebbleRecursiveMutex *s_store_mutex;

static void prv_store_lock(void) {
  mutex_lock_recursive(s_store_mutex);
}

static void prv_store_unlock(void) {
...
static int prv_nimble_store_write_sec(const int obj_type,
  ...
  // The notify handlers take bt_lock, which must not happen while NimBLE
  // holds ble_hs_mutex around this callback: defer them to KernelMain.
...
static int prv_nimble_store_write_cccd(const struct ble_store_value_cccd *value_cccd) {
  ...
  // Persistent storage has its own locking and may call back into
  // bt_driver_handle_host_*() handlers that take the store lock, so it must
  // not be called with the store lock held.
...
static int prv_nimble_store_delete_cccd(const struct ble_store_key_cccd *key_cccd) {
  ...
  // Must be called without the store lock held, see prv_nimble_store_write_cccd()
