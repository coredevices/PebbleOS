...
static void launcher_force_quit_app(void *data) {
  if (s_force_quit_was_cancelled) {
    ...
    PBL_LOG_ERR(
        "NewTimer event fired for force quit, but the back button was released before we went to "
        "deal with it!  Wow, we must have been really slow!  Please file a bug!");
    return;
  }
