...
typedef struct ClickConfig {
  ...
  struct multi_click {
    ...
    uint8_t max;  //!< Maximum number of clicks after which the click counter is reset. The default
                  //!< 0 means use "min" also as "max".
  ...
  struct long_click {
    ...
    ClickHandler release_handler;  //!< In case a long click has been detected, fired when the
                                   //!< button is released.
