...
/*
 * Software SHA-256 (FIPS 180-4). Portable across all platforms.
 *
 * TODO: a hardware-accelerated backend (e.g. the SiFli SF32 HASH unit on the
 * Time 2 / Round 2) can replace this on supported boards; planned as a
 * separate change. This software path stays as the portable fallback.
 *
 * Adapted from Brad Conte's crypto-algorithms
 * (https://github.com/B-Con/crypto-algorithms), which its author dedicated
 * to the public domain. Public-domain material carries no restrictions, so
 * it is provided here under Apache-2.0 as part of this project; the original
 * author is credited above.
 */
...
static void prv_sha256_transform(SHA256Context *ctx, const uint8_t data[]) {
