...
static void prv_up_down_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (data->state == TimelineAppStateNoEvents) {
    if (!next && !data->full_app_view) {
      // We don't want to exit by pressing any key while in the full timeline
      prv_exit(data);
    }
...
static bool NOINLINE prv_setup_timeline_app(void) {
  ...
  *data = (TimelineAppData){};
  ...
  if (uuid_equal(&app_uuid, &(Uuid)TIMELINE_FULL_UUID_INIT) || (args != NULL && args->force_full)) {
    // Activating the full app flag if we entered this code using "timeline full" UUID, or if we entered as
    // "timeline past" UUID and the argument force_full is set.
