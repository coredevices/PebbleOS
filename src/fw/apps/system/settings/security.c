...
// the rest enable it with the matching inactivity duration. There is no
// zero-second entry: a 0 s inactivity timer re-locks before the user can
// navigate (see pin_lock_handle_activity), so the shortest interval is 30 s.
static const uint16_t s_timeout_values[] = {
  0, 30, 60, 180, 300, 600, 900, 1800, 3600,
};
...
static int prv_timeout_index(const SecurityData *data) {
  ...
  return 1;  // an unlisted/legacy duration (e.g. 0) -> default to 30 Seconds
}
...
static void prv_draw_row_cb(SettingsCallbacks *context, GContext *ctx,
  ...
  switch ((SecurityRow)row) {
    ...
    case ROW_TRIGGER_BT:
      ...
      break;
#ifdef CONFIG_HRM
    case ROW_TRIGGER_WRIST_OFF:
      title = i18n_noop("Lock on Wrist Off");
      // Needs the optical HR sensor, gated by Health Tracking and the HR pref.
      subtitle = !sys_activity_prefs_tracking_is_enabled()
                     ? i18n_noop("Requires Health On")
                     : !sys_activity_prefs_heart_rate_is_enabled()
                           ? i18n_noop("Requires Heart Rate")
                           : (data->cfg.trigger_wrist_off ? i18n_noop("On") : i18n_noop("Off"));
      break;
#endif
...
#ifdef CONFIG_HRM
    case ROW_TRIGGER_WRIST_OFF:
      if (!sys_activity_prefs_tracking_is_enabled() ||
          !sys_activity_prefs_heart_rate_is_enabled()) {
        return;  // needs Health Tracking + Heart Rate; the row shows the reason
      }
      data->cfg.trigger_wrist_off = !data->cfg.trigger_wrist_off;
      prv_save_and_reload(data);
      break;
#endif
    case ROW_HIDE_NOTIFS:
