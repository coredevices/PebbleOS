...
#define X_SHIFT (((DISP_COLS - LEGACY_2X_DISP_COLS) / 2) + PBL_IF_ROUND_ELSE(23, 1))
#define Y_SHIFT (((DISP_ROWS - LEGACY_2X_DISP_ROWS) / 2) + PBL_IF_ROUND_ELSE(8, 3))

// Used to shrink the thinkness of the bars
#define X_SHRINK 2

// These are used to shrink the shape for round
#define X_ADJ (PBL_IF_ROUND_ELSE(-12, -3))
#define Y_ADJ (PBL_IF_ROUND_ELSE(-3, 1))
...
#define MASKING_RECT_X_SHIFT (X_SHIFT + 1)
#define MASKING_RECT_Y_SHIFT (Y_SHIFT + 1)
#define MASKING_RECT_X_ADJ (X_ADJ - 1)
#define MASKING_RECT_Y_ADJ (Y_ADJ - 1)
