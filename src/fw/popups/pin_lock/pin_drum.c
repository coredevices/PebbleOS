...
// Shared drum-roll PIN panel widget.
// Draws a bold title and a centred row of rounded panels, one per PIN digit.
// The active panel animates with a vertical drum-roll, like the number wheels
// on an old flip clock. Also draws an animated padlock below the panels.
...
static void prv_roll_update(Animation *animation, const AnimationProgress progress) {
  PinDrum *drum = (PinDrum *)animation_get_context(animation);
  drum->progress = progress;
  layer_mark_dirty(drum->layer);
}

static void prv_roll_stopped(Animation *animation, bool finished, void *context) {
  PinDrum *drum = (PinDrum *)context;
  drum->animating = false;
  drum->anim = NULL;
  // Light "tic" as the drum settles — only on a natural finish (not when a
  // rapid next press interrupts the roll), so holding up/down doesn't buzz-storm.
  if (finished && drum->config.haptic) {
...
static void prv_draw_padlock(GContext *ctx, GRect bounds,
  ...
  int16_t dx = 0;
  if (state == PIN_DRUM_LOCK_SHAKING) {
  ...
  bool clack = false;
  if (state == PIN_DRUM_LOCK_OPENING) {
    const int32_t MAX = ANIMATION_NORMALIZED_MAX;
  ...
  const int16_t lift_r = (state == PIN_DRUM_LOCK_OPENING) ? shackle_lift / 3 : 0;
...
static void prv_shake_stopped(Animation *animation, bool finished, void *context) {
  PinDrum *drum = (PinDrum *)context;
  drum->lock_anim = NULL;
  drum->lock_state = PIN_DRUM_LOCK_CLOSED;
  drum->lock_progress = 0;
  if (drum->layer) {
    layer_mark_dirty(drum->layer);
  }
}
...
static void prv_open_stopped(Animation *animation, bool finished, void *context) {
  ...
  void (*on_done)(void *) = drum->lock_on_open_done;
  void *on_ctx = drum->lock_on_open_ctx;
  drum->lock_on_open_done = NULL;
  drum->lock_on_open_ctx = NULL;
  if (on_done) {
    on_done(on_ctx);
  }
}
...
void pin_drum_init(PinDrum *drum, const PinDrumConfig *config) {
  *drum = (PinDrum){ .config = *config };
}

void pin_drum_reset(PinDrum *drum) {
  if (drum->anim) {
    animation_unschedule(drum->anim);
    animation_destroy(drum->anim);
    drum->anim = NULL;
  }
  drum->animating = false;
  drum->progress = 0;
  ...
  if (drum->lock_anim) {
    animation_unschedule(drum->lock_anim);
    animation_destroy(drum->lock_anim);
    drum->lock_anim = NULL;
  }
  drum->lock_state = PIN_DRUM_LOCK_CLOSED;
  drum->lock_progress = 0;
...
void pin_drum_animate_step(PinDrum *drum, Layer *layer,
  ...
  drum->layer = layer;
  drum->from_digit = from_digit;
  drum->direction = direction;
  drum->progress = 0;
  drum->animating = true;
  ...
  animation_set_handlers(anim, (AnimationHandlers){ .stopped = prv_roll_stopped }, drum);
  drum->anim = anim;
  animation_schedule(anim);
}

void pin_drum_draw(PinDrum *drum, GContext *ctx, GRect bounds) {
  const PinEntry *e = drum->config.entry;
  ...
  graphics_context_set_text_color(ctx, GColorBlack);
  graphics_draw_text(ctx, i18n_get(drum->config.title, drum), title_font, title_box,
                     GTextOverflowModeTrailingEllipsis, GTextAlignmentCenter, NULL);
  i18n_free_all(drum);
  ...
  for (uint8_t i = 0; i < n; i++) {
    ...
    if (i == e->pos) {
      ...
      graphics_context_set_stroke_color(ctx, PBL_IF_COLOR_ELSE(drum->config.accent, GColorBlack));
      ...
      if (drum->animating) {
        ...
        const int16_t off = pin_drum_roll_offset(drum->progress, panel_h);
        // +1 (up): new enters from bottom, old exits to top.
        // -1 (down): new enters from top, old exits to bottom.
        const int16_t new_dy = (int16_t)(drum->direction > 0 ? panel_h - off : off - panel_h);
        const int16_t old_dy = (int16_t)(-drum->direction * off);

        prv_draw_glyph(ctx, &panel, digit_font, (char)('0' + e->digits[i]), new_dy);
        prv_draw_glyph(ctx, &panel, digit_font, (char)('0' + drum->from_digit), old_dy);
      ...
      prv_draw_panel_shell(ctx, &panel);
      const char ch = drum->config.mask_confirmed ? '*' : (char)('0' + e->digits[i]);
  ...
  prv_draw_padlock(ctx, bounds, drum->lock_state, drum->lock_progress);
}
...
void pin_drum_padlock_shake(PinDrum *drum, Layer *layer) {
  ...
  drum->layer = layer;
  drum->lock_state = PIN_DRUM_LOCK_SHAKING;
  drum->lock_progress = 0;
  ...
  animation_set_handlers(anim, (AnimationHandlers){ .stopped = prv_shake_stopped }, drum);
  drum->lock_anim = anim;
  animation_schedule(anim);
}
...
void pin_drum_padlock_open(PinDrum *drum, Layer *layer,
  ...
  drum->layer = layer;
  drum->lock_state = PIN_DRUM_LOCK_OPENING;
  drum->lock_progress = 0;
  drum->lock_on_open_done = on_done;
  drum->lock_on_open_ctx = ctx;
  ...
  animation_set_handlers(anim, (AnimationHandlers){ .stopped = prv_open_stopped }, drum);
  drum->lock_anim = anim;
  animation_schedule(anim);
}
