...
static void prv_update_proc(Layer *layer, GContext *ctx) {
  ...
  s_drum.config.mask_confirmed = pin_lock_should_mask_digits();
  s_drum.config.haptic = pin_lock_should_haptic();
  pin_drum_draw(&s_drum, ctx, GRect(0, 0, DISP_COLS, DISP_ROWS));
}

static void prv_pop(void) {
  pin_drum_reset(&s_drum);
...
static void prv_up_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  pin_drum_animate_step(&s_drum, window_get_root_layer(&s_window), old, +1);
}
...
static void prv_select_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (pin_lock_storage_verify_pin(s_entry.digits, s_entry.len)) {
    pin_drum_padlock_open(&s_drum, window_get_root_layer(&s_window),
    ...
    if (s_drum.anim) {
      animation_unschedule(s_drum.anim);
      animation_destroy(s_drum.anim);
      s_drum.anim = NULL;
    }
    s_drum.animating = false;
    s_drum.progress = 0;
    pin_drum_padlock_shake(&s_drum, window_get_root_layer(&s_window));
    layer_mark_dirty(window_get_root_layer(&s_window));
  }
}
