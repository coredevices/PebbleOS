...
integer printf aliases). picolibc is built from the third_party/picolibc
submodule (or taken pre-built from the SDK). Runs at configure time,
after pebble_arm_gcc has set the architecture flags.
"""
...
def _arch_flags(conf):
    return [f for f in conf.env.CFLAGS
            if any(f.startswith(p) for p in _ARCH_PREFIXES)]
...
def _build_picolibc(conf):
    ...
    if src is None or src.find_node("meson.build") is None:
        conf.fatal("third_party/picolibc submodule missing; run "
    ...
    if not builddir.find_node("build.ninja"):
        ...
        subprocess.check_call(
            [meson, "setup", builddir.abspath(), src.abspath(),
             "--cross-file", out.make_node("cross.txt").abspath(),
             "-Dspecsdir=" + install.make_node("lib").abspath(),
             "--prefix=" + install.abspath()] + _PICOLIBC_MESON_OPTS)
        conf.end_msg("done")
...
def _select_picolibc(conf):
    if conf.env.CONFIG_LIBC_PICOLIBC_FROM_SOURCE:
        specs = _build_picolibc(conf)
    else:
        specs = _find_prebuilt_picolibc(conf)
        if not specs:
            conf.fatal(
                "CONFIG_LIBC_PICOLIBC is set without "
                "CONFIG_LIBC_PICOLIBC_FROM_SOURCE, but no pre-built picolibc "
                "was found in the toolchain. Enable "
                "CONFIG_LIBC_PICOLIBC_FROM_SOURCE to build it from the "
                "third_party/picolibc submodule.")
...
def configure(conf):
    if conf.env.CONFIG_LIBC_PEBBLE:
        ...
        _select_picolibc(conf)
        conf.msg("libc", "picolibc")
    else:
