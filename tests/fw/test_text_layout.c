...
void test_text_layout__cache_vert_overflow_with_newline(void) {
  ...
  cl_assert_equal_i(layout.box.size.w, box.size.w);
  cl_assert_equal_i(layout.max_used_size.w,
                    4 * HORIZ_ADVANCE_PX);  // Includes Whop - as it may be partially rendered at
  ...
  cl_assert_equal_i(layout.box.size.w, box.size.w);
  cl_assert_equal_i(layout.max_used_size.w,
                    2 * HORIZ_ADVANCE_PX);  // Back to only JR - as the line being laid out from
                                            // y=20-30px is empty (and the line from 30-40, Whop, is
                                            // truncated as it can never appear)
