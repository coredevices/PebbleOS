...
void test_pin_lock__zero_timeout_does_not_arm_timer(void) {
  // A 0 s inactivity timeout must not arm a 0 ms timer: it would fire before
  // the user can navigate and re-lock instantly after every unlock.
  const uint8_t pin[4] = {1,2,3,4};
  pin_lock_storage_set_pin(pin, 4);
  PinLockConfig cfg; pin_lock_storage_load(&cfg);
  cfg.trigger_timeout = true; cfg.timeout_s = 0;
  pin_lock_storage_save_config(&cfg);
  pin_lock_init();                 // exercises pin_lock_handle_activity()
  cl_assert_equal_b(false, s_timer_started);
  pin_lock_handle_activity();
  cl_assert_equal_b(false, s_timer_started);
}

void test_pin_lock__nonzero_timeout_arms_timer(void) {
  const uint8_t pin[4] = {1,2,3,4};
  pin_lock_storage_set_pin(pin, 4);
  PinLockConfig cfg; pin_lock_storage_load(&cfg);
  cfg.trigger_timeout = true; cfg.timeout_s = 30;
  pin_lock_storage_save_config(&cfg);
  pin_lock_init();
  cl_assert_equal_b(true, s_timer_started);
  cl_assert_equal_i(30000, s_timer_timeout_ms);
}
