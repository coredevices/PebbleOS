...
int pbl_thread_create(struct pbl_thread *t, const struct pbl_thread_attr *attr) { return 0; }
...
struct pbl_thread *pbl_thread_current(void) { return NULL; }

struct pbl_thread *pbl_thread_idle(void) { return NULL; }

void pbl_thread_prio_set(struct pbl_thread *t, pbl_prio_t prio) {}

pbl_prio_t pbl_thread_prio_get(const struct pbl_thread *t) { return 0; }

enum pbl_thread_state pbl_thread_state(const struct pbl_thread *t) { return PBL_THREAD_READY; }
...
void stub_control_reg(uint32_t reg) { s_app_task_control_reg = reg; }
