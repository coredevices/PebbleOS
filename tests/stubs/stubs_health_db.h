...
int health_db_get_len(const uint8_t *key, int key_len) { return 0; }

status_t health_db_read(const uint8_t *key, int key_len, uint8_t *val_out, int val_out_len) {
  return S_SUCCESS;
}

status_t health_db_delete(const uint8_t *key, int key_len) { return S_SUCCESS; }

status_t health_db_flush(void) { return S_SUCCESS; }

status_t health_db_compact(void) { return S_SUCCESS; }
