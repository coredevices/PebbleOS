...
void rtc_set_time(time_t time) { s_time_base = time; }

void rtc_set_timezone(TimezoneInfo *tzinfo) { s_tzinfo = *tzinfo; }
...
void rtc_get_timezone(TimezoneInfo *tzinfo) { *tzinfo = s_tzinfo; }

void rtc_timezone_clear(void) { memset(&s_tzinfo, 0, sizeof(s_tzinfo)); }

bool rtc_sanitize_struct_tm(struct tm *t) { return false; }

bool rtc_sanitize_time_t(time_t *t) { return rtc_sanitize_struct_tm(NULL); }
...
time_t rtc_get_time(void) { return s_time_base; }

time_t sys_get_time(void) { return rtc_get_time(); }
...
RtcTicks sys_get_ticks(void) { return rtc_get_ticks(); }
...
void fake_rtc_increment_time(time_t inc) { s_time_base += inc; }
...
void fake_rtc_set_ticks(RtcTicks new_ticks) { s_rtc_tick_count = new_ticks; }

void fake_rtc_increment_ticks(RtcTicks tick_increment) { s_rtc_tick_count += tick_increment; }
