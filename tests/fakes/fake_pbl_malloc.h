...
void fake_malloc_set_largest_free_block(size_t bytes) { s_max_size_allowed = bytes; }
...
int fake_pbl_malloc_num_net_allocs(void) { return list_count((ListNode *)s_pointer_list); }
...
void *task_malloc(size_t bytes) { return malloc_and_track(bytes, __builtin_return_address(0)); }
...
void task_free(void *ptr) { free_and_track(ptr); }

void *applib_zalloc(size_t bytes) {
  return calloc_and_track(1, bytes, __builtin_return_address(0));
}

void applib_free(void *ptr) { free_and_track(ptr); }

void *app_malloc(size_t bytes) { return malloc_and_track(bytes, __builtin_return_address(0)); }

void *app_malloc_check(size_t bytes) {
  return malloc_and_track(bytes, __builtin_return_address(0));
}

void app_free(void *ptr) { free_and_track(ptr); }

void *kernel_malloc(size_t bytes) { return malloc_and_track(bytes, __builtin_return_address(0)); }
...
void kernel_free(void *ptr) { free_and_track(ptr); }
...
void smart_free(void *ptr) { free_and_track(ptr); }
